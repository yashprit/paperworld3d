...
package com.paperworld.rpc.scenes 
{
	...
	public class RemoteScene extends Scene3D
	{
		...
		public function RemoteScene(zone:String = null, autoConnect:Boolean = false)
		{
			super( );
		...
		{
			logger.info("RemoteScene ApplicationContext loaded == " + _autoConnect);
		...
		{		
			...
			{			
				logger.info("RemoteScene connected, adding player " + player.username);
				...
				else
				{
					...
					logger.info("avatar: " + player.avatar);
				...
				for (var j : int = 0; j < length ; j++)
				{
					...
					switch (item["code"])
					{
						case "clear":
							...
							{
								logger.info("IN SCENE: " + i + " => " + so.data[i]);
							}
							break;
		...
		protected function createPaperworldObject(key : String, name : String) : Avatar 
		{						
			logger.info("trying to create: " + key + ", " + name + ", " + _so._so.data[name]);
			logger.info("object: " + _appContext.objectDefinitionNames);
			...
			object.name = name;
			object.initialise(_so._so.data[name]);
