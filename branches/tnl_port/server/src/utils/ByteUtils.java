...
public class ByteUtils {
	...
	public static short byteArrayToShort(byte[] paRawBytes, int piOffset,
		...
		// TODO: revisit this: should we silently add missing byte and
		// should
	...
	public static final float byteArrayToFloat(byte[] rawBytes, int offset, boolean pbBigEndian) {
		int iRetVal = 0;
		...
		if (pbBigEndian) {
			iLowest = rawBytes[offset + 3] & 0xff;
			iLow = rawBytes[offset + 2] & 0xff;
			iMid = rawBytes[offset + 1] & 0xff;
			iHigh = rawBytes[offset + 0] & 0xff;
		} else {
			iLowest = rawBytes[offset + 0] & 0xff;
			iLow = rawBytes[offset + 1] & 0xff;
			iMid = rawBytes[offset + 2] & 0xff;
			iHigh = rawBytes[offset + 3] & 0xff;
		}

		iRetVal = iHigh << 24 | iMid << 16 | iLow << 8 | iLowest;
		
		return Float.intBitsToFloat(iRetVal);
